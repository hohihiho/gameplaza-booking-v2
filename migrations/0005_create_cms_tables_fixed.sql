-- CMS 콘텐츠 관리 테이블 (스키마와 일치하도록 수정)
-- 생성일: 2025-09-07

-- content_pages 테이블 (스키마와 일치)
CREATE TABLE IF NOT EXISTS content_pages (
    id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
    slug TEXT NOT NULL UNIQUE,
    title TEXT NOT NULL,
    content TEXT NOT NULL,
    content_type TEXT DEFAULT 'html' CHECK (content_type IN ('markdown', 'html')),
...
CREATE INDEX IF NOT EXISTS idx_content_pages_version ON content_pages(version);
CREATE INDEX IF NOT EXISTS idx_content_pages_created_at ON content_pages(created_at);

-- 초기 콘텐츠 데이터 삽입 (현재 약관 내용)
INSERT OR IGNORE INTO content_pages (slug, title, content, content_type, version, is_published, published_at, created_by, updated_by) VALUES
(
    'terms_of_service',
    '이용약관',
    '<h1>광주 게임플라자 이용약관</h1>
