...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('rental_settings')
      .select('*')
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 먼저 기존 설정이 있는지 확인
    const supabaseAdmin = createAdminClient();
  const { data: existing } = await supabaseAdmin.from('rental_settings')
      .select('*')
  ...
  const { data: deviceType } = await supabaseAdmin.from('device_types')
  ...
  const { error: updateError } = await supabaseAdmin.from('device_types')
      .update({
