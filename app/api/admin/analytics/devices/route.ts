...
export async function GET(request: Request) {
  ...
  const { data: adminData } = await supabaseAdmin.from('admins')
    ...
    const deviceTypeId = searchParams.get('deviceType'); // 특정 기종 필터링
  ...
  const { data: deviceTypesData } = await supabaseAdmin.from('device_types')
    ...
    deviceTypes?.forEach(deviceType => {
      ...
      const completedReservations = deviceReservations.filter(r => 
        r.status === 'approved' || r.status === 'completed' || r.status === 'checked_in'
      );
      const cancelledReservations = deviceReservations.filter(r => r.status === 'cancelled');
      const noShowReservations = deviceReservations.filter(r => r.status === 'no_show');
      
      const totalRevenue = completedReservations.reduce((sum, r) => sum + (r.total_amount || 0), 0);
      const totalRefunds = deviceReservations.reduce((sum, r) => sum + (r.refund_amount || 0), 0);

      // 시간대별 분석
      const hourlyBookings = {};
      deviceReservations.forEach(r => {
        if (r.time_start) {
      ...
      // 크레딧 옵션별 백분율 계산
      const creditAnalysis = Object.entries(creditOptions).map(([type, data]) => ({
        type,
        count: data.count,
    ...
    console.error('기종 분석 API 오류 스택:', error.stack);
    return NextResponse.json({ 
