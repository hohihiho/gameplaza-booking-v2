...
-- 기존 정책 삭제 후 재생성
DROP POLICY IF EXISTS "guide_content_select" ON guide_content;
DROP POLICY IF EXISTS "guide_content_update" ON guide_content;
DROP POLICY IF EXISTS "guide_content_insert" ON guide_content;

-- 모든 사용자가 읽을 수 있음
CREATE POLICY "guide_content_select" ON guide_content
    FOR SELECT USING (true);

-- 관리자만 수정 가능 (NextAuth 사용으로 인해 모든 인증된 사용자 허용, 앱 레벨에서 체크)
...
DROP TRIGGER IF EXISTS update_guide_content_updated_at ON guide_content;
...
CREATE TRIGGER update_guide_content_updated_at BEFORE UPDATE
    ON guide_content 
    FOR EACH ROW 
