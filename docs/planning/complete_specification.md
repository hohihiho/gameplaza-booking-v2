...
### 인증 프로세스
1. **Better Auth 기반 구글 OAuth 로그인** (ONLY 구글)
   - NextAuth v5에서 Better Auth로 마이그레이션 진행 중
   - 세션 기반 인증으로 실시간 권한 확인
   - Cloudflare와 완벽 호환을 위한 Better Auth 도입 예정
   - 현재는 NextAuth v5 사용 중 (차후 Better Auth로 전환)
2. **신규 사용자 회원가입**
   - 닉네임 입력 (필수, 2-8자)
   - 닉네임 중복 체크 및 비속어 필터링
   - 약관 동의 (필수/선택)
   - 이메일 기반 사용자 식별 (재가입 차단용)
3. **인증 완료** → 권한에 따른 기능 이용

### 사용자 권한 시스템
| 권한 | 접근 가능 기능 | 권한 색상 |
|------|----------------|----------|
| **일반 사용자** | 예약 신청, 마이페이지, 정보 조회 | 회색 |
| **제한 유저** | 예약 조회만 가능 (예약 신청 불가, 임시 제한) | 주황색 |
| **정지 유저** | 모든 서비스 이용 불가 (영구 정지) | 빨간색 |
| **스태프** | 예약 조회, 기기현황 확인 (읽기 전용) | 파란색 |
| **관리자** | 모든 기능 + 체크인 + 계좌정보 관리 | 보라색 |
| **슈퍼관리자** | 관리자 권한 + 관리자 추가/삭제 권한 | 금색 |
...
1. **정지 유저 (Banned User)**
   ...
   - 이메일 기반 차단으로 재가입 방지
   - 정지 시 즉시 세션 종료
   - 로그인 시도 시 "정지된 계정입니다" 메시지
   - 슈퍼관리자만 정지 해제 가능

2. **제한 유저 (Restricted User)**
   - 임시적인 서비스 이용 제한
   - 지정된 날짜까지 예약 신청 불가
   - 제한 기간 만료 시 자동으로 일반 유저로 복구
   - 제한 시 즉시 세션 갱신으로 권한 제한
   - 마이페이지에 제한 종료일 표시
   - 관리자가 제한 기간 설정 가능 (예: 7일, 30일 등)

3. **제재 관리 기능**
   - 관리자 페이지에서 사용자 제재 관리
   - 제재 유형: 정지(ban), 제한(restrict), 해제(unban)
   - 제재 사유 기록 및 이력 관리
   - 이메일 블랙리스트 자동 관리
   - 제한 기간 자동 만료 체크 (로그인 시점)
...
### 5. 🏆 대여 랭킹 시스템
- **대여 횟수 기반 랭킹**: 완료된 대여 횟수로 순위 결정
- **랭킹 집계 기준**
  - 체크인 → 결제 완료 → 대여 시간 종료 시 1회 카운트
  - 월간 랭킹 (매월 1일 리셋)
  - 실시간 업데이트
- **랭킹 카테고리**
  - **전체**: 모든 기기 대여 횟수 합산
  - **기기별**: 
    ...
    - 츄니즘 (CHUNITHM)
    - 발키리 (Sound Voltex)
    - 라이트닝 (beatmania IIDX Lightning Model)
  - **시간대별**:
    - 조기 대여 (06:00~12:00 시작)
    - 밤샘 대여 (00:00~05:59 종료)
- **랭킹 표시 정보**
  - 순위 (1-3위는 트로피 아이콘 🥇🥈🥉)
  - 사용자 닉네임
  - 대여 횟수
  - 마지막 대여 일시
- **디자인 통일성**
  - 기존 UI 컴포넌트 재사용 (카드, 버튼, 색상 시스템)
  - 모바일 퍼스트 반응형 디자인
  - 다크모드 지원
- **홈페이지 연동**
  - 메인 페이지 5개 버튼 중 3번째 위치
  - 하단 네비게이션 바에 랭킹 탭 추가
  - 모바일 최적화 UI
...
#### 접근 방법
- **모바일**: 하단 네비게이션 바의 'MY' 탭 클릭
- **데스크톱**: 상단 네비게이션의 프로필 사진 클릭 → 드롭다운 메뉴에서 '마이페이지' 선택
...
#### UI/UX 특징
- **카드형 레이아웃**: 각 섹션별 명확한 구분
- **시각적 통계**: 차트와 그래프로 이용 현황 시각화
- **원터치 액션**: 주요 기능 빠른 접근
...
### V3 시스템 개요
V3는 기존 시스템의 복잡도를 줄이고 핵심 기능에 집중한 단순화된 버전입니다.

### 주요 특징
1. **아키텍처 단순화**
   - Clean Architecture 제거로 불필요한 추상화 제거
   - API Route → Service/Repository → Database 직접 연결
   - 과도한 레이어 제거로 개발 속도 향상

2. **Better Auth 통합**
   - NextAuth 의존성 완전 제거
   - Better Auth로 안정적인 인증 시스템 구현
   - 세션 기반 실시간 권한 확인
   - Google OAuth 단일 인증 지원

3. **사용자 권한 및 제재 시스템**
   - 6단계 권한 시스템 (일반/제한/정지/스태프/관리자/슈퍼관리자)
   - 이메일 기반 블랙리스트로 재가입 차단
   - 임시 제한 기능 (자동 만료)
   - 실시간 세션 갱신으로 즉시 권한 반영
...
- `GET /api/v3/devices` - 기기 목록 조회
- `GET /api/v3/availability` - 예약 가능 여부 확인
- `GET /api/v3/admin` - 관리자 대시보드 데이터
- `POST /api/v3/admin/ban-user` - 사용자 제재 관리
...
│   └── page.tsx              # 메인 페이지
└── components/
    ├── MobileLayout.tsx      # Better Auth 세션 체크
...
**문서 버전**: 5.8 (2025.09.12 업데이트)
**변경 내역**:
- v5.8: V3 시스템 및 Better Auth 통합
  - **V3 예약 시스템 추가**: 단순화된 아키텍처로 핵심 기능 집중
  - **Better Auth 통합**: NextAuth 대체로 안정적인 인증 시스템
  - **사용자 제재 시스템**: 정지/제한 유저 관리 기능 구현
  - **이메일 블랙리스트**: 재가입 방지를 위한 이메일 기반 차단
  - **6단계 권한 시스템**: 일반/제한/정지/스태프/관리자/슈퍼관리자
  - **관리자 대시보드**: V3 전용 관리자 패널 구현
- v5.7: 체크인 및 시간 관리 개선
