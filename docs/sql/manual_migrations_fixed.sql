-- 수동으로 실행해야 하는 마이그레이션 SQL (수정된 버전)
...
CREATE INDEX IF NOT EXISTS idx_time_adjustments_reservation_id ON time_adjustments(reservation_id);
CREATE INDEX IF NOT EXISTS idx_time_adjustments_created_at ON time_adjustments(created_at);
...
CREATE INDEX IF NOT EXISTS idx_amount_adjustments_reservation_id ON amount_adjustments(reservation_id);
CREATE INDEX IF NOT EXISTS idx_amount_adjustments_created_at ON amount_adjustments(created_at);
...
-- 6. 권한 설정 (필요한 경우)
GRANT ALL ON time_adjustments TO authenticated;
GRANT ALL ON amount_adjustments TO authenticated;